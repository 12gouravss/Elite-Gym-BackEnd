...
public interface MemberShipRepo extends JpaRepository<MemberShip, Integer>{
	...
	public List<MemberShipResponse> findALLByUserId(@Param("id") Integer id);
	...
	@Query(value = "SELECT member_ship_id,center,end_date,payment_status,price,start_date,type FROM member_ship WHERE user_id = :id ORDER BY created_at DESC LIMIT 1", nativeQuery = true)
	public MemberShipResponse findByUserId(@Param("id") Integer id);
