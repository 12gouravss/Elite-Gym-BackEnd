...
import in.gourav.elite.DTO.ProductNameCategoryDTO;
import in.gourav.elite.DTO.ratingCountDTO;
import in.gourav.elite.entity.Product;
import in.gourav.elite.request.productUpdateRequest;
...
public interface ProductRepo  extends JpaRepository<Product, Integer>{
 ...
 @Query(value = "SELECT name, category FROM product WHERE lower(name) LIKE CONCAT('%', :filter, '%') OR lower(category) LIKE CONCAT('%', :filter, '%')", 
	       nativeQuery = true)
